...
class GF_Advanced_Post_Creation extends GFFeedAddOn {
	...
	public function scripts() {
		...
		$scripts = array(
			...
			array(
				...
				'src'     => $this->get_base_url() . '/js/taxonomy_map.js',
				'version' => $this->_version,
			),
			array(
				'handle'  => 'gform_advancedpostcreation_form_settings',
			...
			'settings' => array(
				...
				'fields' => array(
					array(
						...
						'tooltip'       => sprintf(
							'<h6>%s</h6>%s',
							esc_html__( 'Post Visiblity', 'gravityformsadvancedpostcreation' ),
	...
	public function get_post_formats_as_choices() {
		...
		$choices = array(
			array(
				'value' => 'standard',
				'label' => esc_html__( 'Standard', 'gravityformsadvancedpostcreation' ),
			),
		);
	...
	public function get_mapped_taxonomies( $feed, $entry, $form ) {
		...
		foreach ( $feed['meta'] as $key => $mappings ) {
			...
			foreach ( $mappings as $mapping ) {
				...
				// Get term based on mapping type.
				switch ( $mapping['key'] ) {

					case 'field':

						// Get mapped field.
						$mapped_field = GFAPI::get_field( $form, $mapping['value'] );

						// Get term IDs for Post Category field.
						if ( $mapped_field && 'post_category' === $mapped_field->type ) {

							// Get field value.
							$field_value = $this->get_field_value( $form, $entry, $mapping['value'] );

							// Get term IDs.
							$term_ids = GFCommon::prepare_post_category_value( $field_value, $mapped_field, 'conditional_logic' );

							// If term IDs were found, add it to array.
							if ( ! empty( $term_ids ) ) {
						...
						// Get comma separated field value.
						$value = $this->get_field_value( $form, $entry, $mapping['value'] );
						$separator = ',';
						/**
						 ...
						 */
						$value = is_array( $value ) ? $value : explode( apply_filters( 'gform_advancedpostcreation_term_separator', $separator ), $value );

						// Get terms for each field value.
						foreach ( $value as $val ) {

							// Look for existing term.
							$existing_term = get_term_by( 'name', $val, $taxonomy );

							// If term exists, add it to array. Otherwise, create term.
							if ( $existing_term ) {

								$terms[] = $existing_term->term_id;
								...
								// If term was created, add it to array.
					...
					case 'term':
						...
						// Process custom term value separately.
						if ( 'gf_custom' === $mapping['value'] ) {

							// Look for existing term.
							$existing_term = get_term_by( 'name', $mapping['custom_value'], $taxonomy );
							...
							if ( $existing_term ) {
								...
								// Create term.
								$new_term = wp_insert_term( $mapping['custom_value'], $taxonomy );
							...
							// Get term.
							$term = get_term_by( 'slug', $mapping['value'], $taxonomy );

							// If existing term was found, add it to array.
							if ( $term ) {
								$terms[] = $term->term_id;
							}
					...
					default:
						...
						// Get mapping value.
						$mapping_value = rgar( $mapping, 'custom_value' ) ? $mapping['custom_value'] : $mapping['value'];

						// Look for existing term.
						$existing_term = get_term_by( 'name', $mapping_value, $taxonomy );
						...
						if ( $existing_term ) {
							...
							// Create term.
							$new_term = wp_insert_term( $mapping_value, $taxonomy );
	...
	private function media_handle_upload( $file_url, $post_id ) {
		...
		// Upload file.
		$media_id = GFFormsModel::media_handle_upload( $file_url, $post_id );
	...
	public function set_post_data( $post, $feed, $entry, $form ) {
		...
		// Set the post author.
		if ( 'logged-in-user' === rgars( $feed, 'meta/postAuthor' ) ) {
			$post['post_author'] = rgar( $entry, 'created_by' );
		} else {
			$post['post_author'] = rgars( $feed, 'meta/postAuthor' );
		}
	...
	public function replace_media_merge_tag( $text, $form, $entry, $url_encode = false, $esc_html = true, $nl2br = true, $format = 'html' ) {
		...
		foreach ( $matches as $match ) {
			...
			// Get feed ID and modifier.
			$feed_id  = rgar( $match, 1 ) && is_numeric( $match[1] ) ? $match[1] : $this->_current_feed;
			$modifier = rgar( $match, 2 ) ? $match[2] : 'ids';

			// Set media files to false.
			$media = false;

			// Get media based on feed ID.
			if ( $this->_current_feed === $feed_id ) {

				// Get currently submitted media files.
				$media = $this->get_current_media();

			} else {

				// Get entry meta.
				$meta = gform_get_meta( $entry['id'], $this->_slug . '_post_id' );

				// Loop through meta items to get feed.
				foreach ( $meta as $m ) {
					if ( $feed_id == $m['feed_id'] ) {
						$media = $m['media'];
					}
				}
