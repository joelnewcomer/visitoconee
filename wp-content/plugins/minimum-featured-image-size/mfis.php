...
/**
 * Enqueue css for setting page
 * @since 1.0
 */
function mfis_enqueue_admin_css() {

    # admin css
    wp_enqueue_style( 'mfis-admin-css', plugins_url('admin/mfis-admin-styles.css', __FILE__ ));
...
/**
 * Enqueue css and js for ajax functionality
 * @since 1.0
 */
function mfis_enqueue_admin_ajax() {
    ...
    # admin css
    wp_enqueue_style( 'mfis-admin-ajax-css', plugins_url('admin/mfis-admin-ajax-styles.css', __FILE__ ));
		...
		<form method="post" action="options.php">
			...
			<table class="form-table" id="mfis_custom_posts_options_table">
				...
				foreach ( $post_types as $post_type ) {
					...
					<tr valign="middle">
						<th valign="middle" scope="row"><?php echo $post_type->labels->singular_name; ?></th>
						<td valign="middle">
							<table>
								<tr>							
									<td valign="middle">
...
function mfis_after_save($new_status, $old_status, $post){
	...
	if( ! mfis_image_size_ok( $image_id, $post_type ) ){
		...
		if ( $post_type === 'post' ) :
			...
			$min_width = get_option( 'mfis_min_width_' . $post_type );
			$min_height = get_option( 'mfis_min_height_' . $post_type );

			$min_width = ! empty($min_width) && get_option('mfis_custom_post_types') === 'mfis_custom_post_types' ? $min_width : get_option( 'mfis_min_width' );
			$min_height = ! empty($min_height) && get_option('mfis_custom_post_types') === 'mfis_custom_post_types' ? $min_height : get_option( 'mfis_min_height' );
...
if (is_admin()) add_action('transition_post_status', 'mfis_after_save', 10, 3);
